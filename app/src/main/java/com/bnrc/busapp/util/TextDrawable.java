...
public class TextDrawable extends ShapeDrawable {
	...
	private TextDrawable(Builder builder) {
		...
		// shape properties
		shape = builder.shape;// �?么形�?
		height = builder.height;// 高度
		width = builder.width;// 宽度
		radius = builder.radius;// 角度

		// text and color
		text = builder.toUpperCase ? builder.text.toUpperCase() : builder.text;// 文字，是否大�?
		color = builder.color;// 颜色

		// text paint settings
		fontSize = builder.fontSize;// 字体大小
		textPaint = new Paint();// 为文字建立的�?
		textPaint.setColor(builder.textColor);// 文字的颜�?
		textPaint.setAntiAlias(true);// 去锯�?
		...
		borderThickness = builder.borderThickness;// shape边界的线�?
	...
	public static class Builder implements IConfigBuilder, IShapeBuilder,
		...
		public IBuilder roundRect(int radius) {
			this.radius = radius;
			float[] radii = { radius, radius, radius, radius, radius, radius,
					radius, radius };
